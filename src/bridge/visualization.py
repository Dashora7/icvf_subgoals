...
def make_visual(images, metrics, what_to_visualize=[]):
    
    w, h = most_squarelike(len(what_to_visualize))
    ...
    for i in range(len(what_to_visualize)):
        if isinstance(what_to_visualize[i], str):
            method = ALL_VISUALIZATIONS[what_to_visualize[i]]
        else:
            method = what_to_visualize[i]
...
ALL_VISUALIZATIONS = {
    'rewards': ft.partial(visualize_metric, metric_name='rewards'),
}